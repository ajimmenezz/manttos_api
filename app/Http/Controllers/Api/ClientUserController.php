...
class ClientUserController extends Controller
{
    ...
    public function store(Request $request, Client $client): JsonResponse
    {
        ...
        if (! $user->hasRole('admin-cliente')) {
            return response()->json(['message' => 'El usuario debe tener el rol de Administrador de Cliente para ser asignado.'], 422);
        }

        if ($client->admins()->where('user_id', $user->id)->exists()) {
            return response()->json(['message' => 'El usuario ya está asignado a este cliente.'], 422);
        }

        $client->admins()->attach($user->id);

        return response()->json([
            'message' => 'Administrador asignado al cliente correctamente.',
            'user'    => [
                'id'        => $user->id,
                'name'      => $user->name,
                'email'     => $user->email,
    ...
    public function destroy(Request $request, Client $client, User $user): JsonResponse
    {
        ...
        $client->admins()->detach($user->id);

        return response()->json(['message' => 'Administrador removido del cliente.']);
    }
